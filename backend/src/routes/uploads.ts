...
export default async function uploadsRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/api/uploads', { preHandler: requireAdmin }, async (req: FastifyRequest, reply: FastifyReply) => {
    ...
    for await (const part of parts) {
      ...
      if (!(lower.endsWith('.xlsx') || lower.endsWith('.xls') || lower.endsWith('.xlsm') || lower.endsWith('.pdf'))) {
        return reply.status(400).send({ error: 'Only .xls/.xlsx/.xlsm/.pdf files are allowed' })
      }
      const chunks: Buffer[] = []
      // @ts-ignore 
